...
import {
  ...
  PawPrint, Clock, Bot, ArrowLeft, Camera, Images
} from 'lucide-react'
  ...
  async function doSubmit() {
    ...
    if (error) {
      ...
      if (user?.id) {
        ...
        //
        // ⚠️ ชื่อฟังก์ชันต้องไม่เกิน 21 ตัวอักษรไทย
        // PostgreSQL จำกัดชื่อ identifier ไว้ที่ 63 ไบต์ และตัวอักษรไทยกินตัวละ 3 ไบต์
        // ชื่อเดิม 'แจ้งเตือนเจ้าหน้าที่เคสใหม่' ยาว 27 ตัว = 81 ไบต์ เกินขีดจำกัด
        // Postgres จะตัดทิ้งเงียบๆ เหลือ 'แจ้งเตือนเจ้าหน้าที่เ' (21 ตัว)
        // แต่ PostgREST เทียบชื่อจาก URL แบบเต็ม จึงหาไม่เจอและคืน 404 (PGRST202)
        // ผลคือเจ้าหน้าที่ไม่เคยได้รับแจ้งเตือนเลย โดยไม่มี error ให้ผู้ใช้เห็น
        const { error: แจ้งเจ้าหน้าที่พลาด } = await supabase
          .rpc('แจ้งเตือนเคสใหม่', { id_รายงาน: data.id })
      ...
      {แสดงกล้อง && (
        <div ref={ชีตกล้อง} className="fixed inset-0 z-50 bg-black flex flex-col">
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-black/60">
            ...
            <p className="text-white font-medium text-sm flex items-center gap-1.5"><Camera size={14} className="shrink-0" /> ถ่ายรูปสัตว์</p>
            <div className="w-16" />
          </div>
      ...
      <div className="px-4 pt-5 space-y-5">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mb-2 flex items-center gap-1.5">
            <Camera size={12} className="shrink-0" /> ถ่ายให้สัตว์อยู่กลางภาพและเต็มเฟรม จะช่วยให้ AI วิเคราะห์ได้แม่นยำขึ้น
          </p>
          ...
          {รูปภาพPreview ? (
            ...
            <div className="relative rounded-2xl overflow-hidden border-2 border-orange-200">
              ...
              <div className="absolute bottom-0 inset-x-0 bg-black/50 flex items-center justify-center gap-3 px-5 py-3">
                <button onClick={เปิดกล้อง}
                  ...
                  <Camera size={18} className="shrink-0" /> ถ่ายรูป
                </button>
                <span className="w-px h-4 bg-white/30" />
                <button onClick={() => inputGallery.current.click()}
                  className="flex items-center gap-1.5 text-white text-sm font-medium">
                  <Images size={18} className="shrink-0" /> เลือกจากคลัง
                </button>
              </div>
            </div>
          ) : (
            ...
            <div className="relative">
              ...
              >
                <Camera size={56} strokeWidth={1.5} className="text-orange-300 mb-3" />
              ...
              >
                <Images size={16} className="shrink-0" /> คลัง
              </button>
            </div>
          )}
        ...
        {ผลAI && !กำลังวิเคราะห์ && (
          <div className="bg-white border border-orange-200 rounded-2xl p-4 shadow-sm">
            ...
            {/* พิมพ์แล้วมีตัวเลือกสายพันธุ์ที่โมเดลรู้จัก (47 สายพันธุ์) ขึ้นให้เลือก เลือกแล้วขนาด/นิสัยจะอัปเดตตาม */}
