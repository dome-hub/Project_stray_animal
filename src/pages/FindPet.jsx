...
function FindPet() {
  ...
  useEffect(function () {
    async function ดึงข้อมูลสัตว์() {
      ...
      // เฉพาะสัตว์ที่เจ้าหน้าที่คัดกรองและกดเผยแพร่แล้ว "และ" สถานะพร้อมหาบ้านจริงเท่านั้น (double-check
      // คู่กับ auto-toggle-off ฝั่งฟอร์มแก้ไข กันสัตว์ที่รับเลี้ยงแล้ว/ยังไม่พร้อม หลุดมาโชว์)
      const { data, error } = await supabase
        .from('animals')
        .select('*')
        .eq('is_adoptable', true)
        .eq('status', 'รอการรับเลี้ยง')
